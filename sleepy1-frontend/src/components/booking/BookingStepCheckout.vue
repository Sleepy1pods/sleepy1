...
import { formatInr, formatDateLong, formatTime12h } from '@/utils/format'
      ...
      <div class="space-y-6">
        ...
        <div class="card-surface overflow-hidden">
          <div class="flex items-center justify-between border-b border-white/10 bg-white/[0.03] px-6 py-4">
            <span class="text-sm font-semibold uppercase tracking-wide text-white">Booking Summary</span>
          ...
          <div class="p-6 space-y-4">
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <p class="text-xs text-ivory-100/60">{{ flow.selectedLocation?.terminal || 'Platform 1' }} · {{ flow.selectedLocation?.city || 'Dharwad' }}</p>
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 rounded-xl bg-white/[0.03] p-4 text-xs">
              ...
              <div>
                ...
                <span class="mt-0.5 block font-semibold text-ivory-50">{{ formatTime12h(flow.draft.checkIn) || 'Not selected' }}</span>
              </div>
              <div>
            ...
            <div class="border-t border-white/10 pt-4 text-xs">
              <div class="flex items-center justify-between">
                <span class="text-ivory-100/50">Guest & Schedule Details</span>
              ...
              <div class="mt-2 text-ivory-50">
                ...
                <p class="mt-1 text-ivory-100/60">
                  ...
                  <span v-if="flow.draft.guest?.podCode" class="ml-2 font-semibold text-white">· Pod Code: {{ flow.draft.guest.podCode }}</span>
                </p>
              </div>
            </div>
          </div>
        ...
        <div class="space-y-3">
          ...
          <button
            ...
            :class="flow.draft.paymentMethod === method.id ? 'border-white ring-2 ring-white/25 bg-white/10' : ''"
            :aria-pressed="flow.draft.paymentMethod === method.id"
            @click="selectMethod(method.id)"
          >
            <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full border mt-0.5" :class="flow.draft.paymentMethod === method.id ? 'border-white bg-white' : 'border-white/25'">
              <span v-if="flow.draft.paymentMethod === method.id" class="h-2 w-2 rounded-full bg-black" />
            </span>
            <div class="flex-1">
              <div class="flex items-center justify-between">
          ...
          <div v-if="flow.draft.paymentMethod !== 'direct' && flow.draft.paymentMethod !== 'razorpay'" class="card-surface p-5">
            ...
            <label for="credits-range" class="mb-2 mt-4 block text-sm font-medium text-ivory-100/70">
              Credits to apply: <span class="font-bold text-white">{{ flow.draft.creditsToApply.toLocaleString('en-IN') }}</span>
            </label>
            <input
              id="credits-range"
              type="range"
        ...
        <div class="sticky top-24">
          ...
          <div class="mt-4">
            <PrimaryButton
              ...
              @click="payNow"
            >
              <span>Confirm & Book Pod ({{ formatInr(flow.pricing.totalPayable) }}) →</span>
            </PrimaryButton>
          </div>
